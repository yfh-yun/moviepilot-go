...
type Notification struct {
	...
	Source string `json:"source,omitempty"`
	// 消息类型
	MType NotificationType `json:"mtype,omitempty"`
	// 内容类型
	CType ContentType `json:"ctype,omitempty"`
	// 标题
...
type ChannelCapabilityManager struct {
	capabilities map[MessageChannel]*ChannelCapabilities
}
...
func NewChannelCapabilityManager() *ChannelCapabilityManager {
	manager := &ChannelCapabilityManager{
		capabilities: make(map[MessageChannel]*ChannelCapabilities),
	}

	// Telegram配置
	manager.capabilities[MessageChannelTelegram] = &ChannelCapabilities{
...
func (m *ChannelCapabilityManager) GetCapabilities(channel MessageChannel) *ChannelCapabilities {
	return m.capabilities[channel]
}

// SupportsCapability 检查渠道是否支持某项能力
func (m *ChannelCapabilityManager) SupportsCapability(channel MessageChannel, capability ChannelCapability) bool {
...
func (m *ChannelCapabilityManager) GetMaxButtonRows(channel MessageChannel) int {
	caps := m.GetCapabilities(channel)
	if caps == nil {
		return 5
	}
...
func (m *ChannelCapabilityManager) GetMaxButtonTextLength(channel MessageChannel) int {
	caps := m.GetCapabilities(channel)
	if caps == nil {
		return 20
	}
...
func (m *ChannelCapabilityManager) ShouldUseFallback(channel MessageChannel) bool {
	caps := m.GetCapabilities(channel)
	if caps == nil {
		return true
	}
