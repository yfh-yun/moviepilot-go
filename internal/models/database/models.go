...
type Subscribe struct {
	...
	SearchIMDBID       int        `gorm:"column:search_imdbid;default:0" json:"search_imdbid"`               // 是否使用 imdbid 搜索
	ManualTotalEpisode int        `gorm:"column:manual_total_episode;default:0" json:"manual_total_episode"` // 是否手动修改过总集数
	CustomWords        string     `gorm:"column:custom_words;size:500" json:"custom_words"`                  // 自定义识别词
	FilterGroups       string     `gorm:"column:filter_groups;type:json" json:"filter_groups"`               // 过滤规则组
	Date               string     `gorm:"column:date;size:50" json:"date"`                                   // 创建时间
}
...
type Download struct {
	BaseModel
	Hash          string     `gorm:"size:100;not null;uniqueIndex" json:"hash"`
	Title         string     `gorm:"size:500;not null" json:"title"`
	Size          int64      `json:"size"`
	Progress      float64    `json:"progress"`
	Status        string     `gorm:"size:20;not null;index" json:"status"`
	Downloader    string     `gorm:"size:50;not null" json:"downloader"` // qbittorrent, transmission
	SavePath      string     `gorm:"size:500" json:"save_path"`
	Category      string     `gorm:"size:100" json:"category"`
	Tags          string     `gorm:"type:json" json:"tags"`
	TorrentURL    string     `gorm:"type:text" json:"torrent_url"`
	SubscribeID   *uint      `gorm:"index" json:"subscribe_id"`
...
type TransferHistory struct {
	BaseModel
	// 源信息
	Src         string `gorm:"column:src;size:1000;not null;index" json:"src"`           // 源路径
	SrcStorage  string `gorm:"column:src_storage;size:50" json:"src_storage"`            // 源存储
	SrcFileItem string `gorm:"column:src_fileitem;type:json" json:"src_fileitem"`        // 源文件项
	Source      string `gorm:"size:500;not null" json:"source"`                          // 源 (兼容字段)
	SourcePath  string `gorm:"column:source_path;size:1000;not null" json:"source_path"` // 源路径 (兼容字段)

	// 目标信息
	Dest         string `gorm:"column:dest;size:1000;not null" json:"dest"`               // 目标路径
	DestStorage  string `gorm:"column:dest_storage;size:50" json:"dest_storage"`          // 目标存储
	DestFileItem string `gorm:"column:dest_fileitem;type:json" json:"dest_fileitem"`      // 目标文件项
	Target       string `gorm:"size:500;not null" json:"target"`                          // 目标 (兼容字段)
	TargetPath   string `gorm:"column:target_path;size:1000;not null" json:"target_path"` // 目标路径 (兼容字段)

	// 转移信息
	Mode     string `gorm:"size:20" json:"mode"`                      // 转移模式 move/copy/link/hardlink
	Type     string `gorm:"size:20;not null" json:"type"`             // 类型 电影/电视剧
	Category string `gorm:"column:category;size:100" json:"category"` // 二级分类

	// 媒体信息
	Title        string  `gorm:"size:500;not null;index" json:"title"`               // 标题
	Year         *string `gorm:"size:10" json:"year"`                                // 年份
	TMDBID       *int    `gorm:"index" json:"tmdb_id"`                               // TMDB ID
	IMDBID       *string `gorm:"size:20" json:"imdb_id"`                             // IMDB ID
	TVDBID       *int    `gorm:"index" json:"tvdb_id"`                               // TVDB ID
	DoubanID     *string `gorm:"size:50" json:"douban_id"`                           // 豆瓣 ID
	Seasons      string  `gorm:"size:100" json:"seasons"`                            // 季 Sxx
	Episodes     string  `gorm:"size:500" json:"episodes"`                           // 集 Exx
	Image        string  `gorm:"size:500" json:"image"`                              // 海报
	EpisodeGroup string  `gorm:"column:episode_group;size:100" json:"episode_group"` // 剧集组

	// 下载信息
	Downloader   string `gorm:"size:50" json:"downloader"`                                // 下载器
	DownloadHash string `gorm:"column:download_hash;size:100;index" json:"download_hash"` // 下载哈希

	// 状态信息
	Status bool   `gorm:"default:true" json:"status"`            // 转移成功状态
	ErrMsg string `gorm:"column:errmsg;type:text" json:"errmsg"` // 转移失败信息
	Date   string `gorm:"size:50;index" json:"date"`             // 时间

	// 文件清单
	Files string `gorm:"type:json" json:"files"` // 文件清单
	...
	MediaCategory string `gorm:"column:media_category;size:100" json:"media_category"` // 媒体分类
}
...
type Site struct {
	...
	UA            string `gorm:"column:ua;size:500" json:"ua"`                          // User-Agent (兼容Python)
...
type MediaInfo struct {
	...
	Extra         map[string]any `gorm:"type:json" json:"extra"`
}
...
// DownloadFile 下载文件表 (对应Python中的DownloadFiles)
type DownloadFile struct {
	BaseModel
	DownloadHash string `gorm:"column:download_hash;size:100;not null;index" json:"download_hash"` // 下载任务Hash
	FullPath     string `gorm:"column:fullpath;size:1000;not null;index" json:"fullpath"`          // 文件完整路径
	SavePath     string `gorm:"column:savepath;size:1000;not null;index" json:"savepath"`          // 保存路径
	FileName     string `gorm:"column:filename;size:500;not null" json:"filename"`                 // 文件名
	FileSize     int64  `gorm:"column:filesize;default:0" json:"filesize"`                         // 文件大小
	FileExt      string `gorm:"column:fileext;size:20" json:"fileext"`                             // 文件扩展名
	State        int    `gorm:"default:0;index" json:"state"`                                      // 状态 0-正常 1-已删除
}
