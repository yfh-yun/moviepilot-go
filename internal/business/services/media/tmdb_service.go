...
import (
	...
	"moviepilot-go/internal/business/services/media/tmdb"
...
func (s *TMDBService) Identify(files []FileItem, opts IdentifyOptions) ([]database.Media, error) {
	results := make([]database.Media, 0, len(files))
...
func (s *TMDBService) identifySingle(file FileItem) (database.Media, error) {
...
func (s *TMDBService) lookupTMDB(query, typeHint string) (database.Media, error) {
	...
	if err != nil {
		return database.Media{}, fmt.Errorf("tmdb lookup failed: %w", err)
	}

	return media, nil
}

func (s *TMDBService) searchMovie(ctx context.Context, query string) (database.Media, error) {
	// 尝试从文件名提取年份
	...
	if len(resp.Results) == 0 {
		return database.Media{}, fmt.Errorf("no movie found for query: %s", query)
	}
...
func (s *TMDBService) searchTV(ctx context.Context, query string) (database.Media, error) {
	year := extractYearFromQuery(query)
	...
	if len(resp.Results) == 0 {
		return database.Media{}, fmt.Errorf("no TV show found for query: %s", query)
	}
...
func (s *TMDBService) searchMulti(ctx context.Context, query, typeHint string) (database.Media, error) {
	...
	if len(resp.Results) == 0 {
		return database.Media{}, fmt.Errorf("no media found for query: %s", query)
	}
...
func (s *TMDBService) movieResultToMedia(result *tmdb.MovieResult) database.Media {
...
func (s *TMDBService) movieDetailsToMedia(details *tmdb.MovieDetails) database.Media {
...
func (s *TMDBService) tvResultToMedia(result *tmdb.TVResult) database.Media {
...
func (s *TMDBService) tvDetailsToMedia(details *tmdb.TVDetails) database.Media {
...
func (s *TMDBService) multiResultToMedia(result *tmdb.MultiResult) database.Media {
	year := extractYear(result.ReleaseDate)
	if year == "" {
		year = extractYear(result.FirstAirDate)
	}
...
func (s *TMDBService) fallbackMedia(file FileItem, opts IdentifyOptions) database.Media {
	if s.fallback != nil {
...
func (s *TMDBService) DownloadPoster(ctx context.Context, media database.Media, outputDir string, opts tmdb.DownloadOptions) (string, error) {
...
func (s *TMDBService) DownloadBackdrop(ctx context.Context, media database.Media, outputDir string, opts tmdb.DownloadOptions) (string, error) {
...
func (s *TMDBService) DownloadAllImages(ctx context.Context, media database.Media, outputDir string, opts tmdb.DownloadOptions) ([]string, error) {
	var results []string
