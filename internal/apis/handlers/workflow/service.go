...
type workflowManager interface {
	RegisterWorkflow(workflow any) error
	RunWorkflow(id string, ctx context.Context, variables map[string]any) error
	WaitForWorkflow(id string) error
	GetWorkflowResult(id string) (any, error)
}
...
func NewService(manager workflowManager, _ any, log *zap.Logger) *Service {
	if log == nil {
