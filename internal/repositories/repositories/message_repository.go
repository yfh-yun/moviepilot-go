...
func (r *MessageRepositoryImpl) Delete(ctx context.Context, id string) error {
	return r.db.WithContext(ctx).Delete(&database.Message{}, "id = ?", id).Error
}
...
func (r *MessageRepositoryImpl) MarkAsRead(ctx context.Context, id string) error {
	return r.db.WithContext(ctx).Model(&database.Message{}).Where("id = ?", id).Update("status", "read").Error
}

// MarkAllAsRead 标记所有消息为已读
func (r *MessageRepositoryImpl) MarkAllAsRead(ctx context.Context, userID string) error {
	return r.db.WithContext(ctx).Model(&database.Message{}).Where("user_id = ? AND status != ?", userID, "read").Update("status", "read").Error
}
