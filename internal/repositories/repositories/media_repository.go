...
// Create 创建媒体记录
func (r *MediaRepositoryImpl) Create(ctx context.Context, media *database.Media) error {
	return r.db.WithContext(ctx).Create(media).Error
}

// GetByID 根据ID获取媒体
func (r *MediaRepositoryImpl) GetByID(ctx context.Context, id string) (*database.Media, error) {
...
// Update 更新媒体
func (r *MediaRepositoryImpl) Update(ctx context.Context, media *database.Media) error {
	return r.db.WithContext(ctx).Save(media).Error
}

// Delete 删除媒体
func (r *MediaRepositoryImpl) Delete(ctx context.Context, id string) error {
	return r.db.WithContext(ctx).Delete(&database.Media{}, "id = ?", id).Error
}

// List 获取媒体列表
func (r *MediaRepositoryImpl) List(ctx context.Context, params interfaces.ListMediaParams) ([]*database.Media, int64, error) {
	var medias []*database.Media
	var total int64

	query := r.db.WithContext(ctx).Model(&database.Media{})
...
// Search 搜索媒体
func (r *MediaRepositoryImpl) Search(ctx context.Context, query string, params interfaces.SearchMediaParams) ([]*database.Media, error) {
	var medias []*database.Media
...
func (r *MediaRepositoryImpl) GetByTMDBID(ctx context.Context, tmdbID int64, mediaType string) (*database.Media, error) {
	var media database.Media
...
// GetByTitle 根据标题获取媒体
func (r *MediaRepositoryImpl) GetByTitle(ctx context.Context, title string, year int) (*database.Media, error) {
	var media database.Media
